...
[![Tests](https://img.shields.io/badge/Tests-331%20backend%20%2B%20168%20frontend-brightgreen.svg)](backend/tests/)
[![Frontend Coverage](https://img.shields.io/badge/Frontend%20Coverage-Stmts%20100%25-brightgreen.svg)](frontend/)
...
| **Phase 44** | Helm chart README.md整備 | ✅ 完了 | ArtifactHub連携・values-prod.yaml使用ガイド・アーキテクチャ図・SLO/SLI設定ドキュメント・GitHub Release v1.3.0 (#68) |
| **Phase 45** | Kyverno Policy Engine 統合 | ✅ 完了 | 5 ClusterPolicy（disallow-latest-tag / require-pod-security / disallow-privileged / require-ro-rootfs / disallow-host-namespaces）Pod層 Zero Trust 強化・Codex/CodeRabbit 二重レビューで init/ephemeralContainers 全種別カバー（debug コンテナ経由バイパス遮断）・Helm chart v1.4.0 (#69 / #71 / #72) |
| **Phase 45.5** | readOnlyRootFilesystem 全 Pod 適用 + Kyverno Enforce | ✅ 完了 | Backend/Frontend/Loki を `readOnlyRootFilesystem: true` 化（emptyDir mount）・values-prod.yaml を Enforce + requireReadOnlyRootFilesystem=true に昇格・Helm chart v1.5.0 (#73 / #78) |
| **Phase 46** | Sigstore Cosign 署名統合 | ✅ 完了 | Helm chart OCI artifact を Cosign keyless 署名（GitHub Actions OIDC / Fulcio / Rekor）・secret 不要・helm-publish に sign + verify self-check 追加・Helm chart v1.6.0 (#80) |
| **Phase 47** | Falco Runtime Security | ✅ 完了 | eBPF syscall 監視 DaemonSet・ZTI カスタムルール 3 件（shell in container / write below /etc / unexpected inbound listen）・Kyverno と補完的な runtime detection・Helm chart v1.7.0 (#83) |
| **Phase 48** | Velero Backup/Restore (DR 基盤) | ✅ 完了 | Velero Schedule CRD 2 本（日次 full backup 24h RPO + 6 時間毎 DB snapshot 6h RPO）・ISO20000 / NIST CSF RC.RP 準拠・Helm chart v1.8.0 (#88) |
| **Phase 47.1** | Falcosidekick fan-out | ✅ 完了 | Falco JSON stdout を Alertmanager / Slack に転送・Falco http_output 自動配線（conditional）・Helm chart v1.9.0 (#92) |
| **CI 強化** | helm lint values-prod 検証 + alertmanager 再帰修正 | ✅ 完了 | values-prod.yaml CI lint step 追加・`checksum/config` 値ハッシュ化で無限 include 再帰解消 (#70 / #75) |
...
## 👥 Agent Teams (ClaudeOS v8)

本プロジェクトは ClaudeOS v8 Kernel（Autonomous Operations Edition + CodeRabbit Review Integration）による自律開発で構築されています。
...
| 本番設定 | **values-prod.yaml**（セキュリティ全有効化・本番リソース設定・Azure Key Vault必須・NetworkPolicy/SLO/ResourceQuota有効） | Phase 43 ✅ |
| Pod層 Policy as Code | **Kyverno 5 ClusterPolicy**（disallow-latest-tag / require-pod-security / disallow-privileged / require-ro-rootfs / disallow-host-namespaces・containers+initContainers+ephemeralContainers 全種別カバー） | Phase 45 ✅ |
| readOnlyRootFilesystem | **全 Pod Enforce**（Backend/Frontend/Loki/Falco に emptyDir mount・Kyverno Enforce モード本番適用） | Phase 45.5 ✅ |
| サプライチェーン署名 | **Sigstore Cosign keyless**（GitHub Actions OIDC → Fulcio → Rekor transparency log・secret 不要・ghcr.io chart に自動署名+verify self-check） | Phase 46 ✅ |
| ランタイム脅威検知 | **Falco eBPF DaemonSet**（modern_bpf driver・ZTI 3 カスタムルール: shell in container / write below /etc / unexpected listen） | Phase 47 ✅ |
| DR / Backup | **Velero Schedule CRD 2 本**（daily-full 24h RPO / db-snapshot 6h RPO・PostgreSQL PVC label 選択・ISO20000 RC.RP 準拠） | Phase 48 ✅ |
| 脅威通知 fan-out | **Falcosidekick**（Falco JSON → Alertmanager/Slack/Teams 等・自動配線・minimumPriority フィルタ・envFrom ConfigMap） | Phase 47.1 ✅ |
...
*🤖 Built with [ClaudeOS v8](https://claude.ai/claude-code) × GitHub Actions — Phase 1〜47.1/48 完了・STABLE N=16+ 達成・本番脆弱性 0件・フロントエンド Statements **100%** 達成・**Helm chart v1.9.0**（Falcosidekick fan-out + Velero DR + Falco Runtime Security + Sigstore Cosign 署名 + Kyverno Enforce + readOnlyRootFilesystem）・CI 9ジョブ対応・Prometheus + OTEL + Grafana + Loki Observability 5本柱 + Alertmanager アラート通知 + NetworkPolicy Zero Trust ネットワーク・ESO Azure Key Vault Secrets管理・**PodDisruptionBudget + HPA behavior + GitOps ArgoCD/Flux + Helm OCI ghcr.io publish + ResourceQuota/LimitRange 本番リソース管理 + SLO/SLI Multi-Window Burn Rate 監視 + 本番推奨設定ファイル***
